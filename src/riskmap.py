...
색 설계 (dataviz 기준)
    8월 개념문서 9절의 **녹색 → 노랑 → 주황 → 빨강** 4단계 관행을 따른다.

    이 램프는 **명도 단조 감소가 구조적으로 불가능**하다. 가운데 노랑이 양 끝보다
    밝기 때문이다. 따라서 색각 이상에서 명도만으로 순서를 읽을 수 없다.
    실측(dataviz 검증기, deutan):
        안전 #1a9850  vs  심각 #a50026 (진한 적색)  ΔE 13.0   양호
        안전 #1a9850  vs  중간 적색 #d73027         ΔE  5.1   미흡
    그래서 램프 상단을 **진한 적색(#a50026)** 으로 끝내 안전-심각 구분을 확보했다.
    중간값끼리의 혼동은 남는다.

    완화책 — 색에만 의존하지 않는다.
        · 모든 노드에 복합위험도 수치를 직접 표기
        · 우측 순위 패널에 수치와 직접위험도를 병기
...
RISK_RAMP = [
    "#1a9850", "#52b151", "#86c96a", "#b7e07d", "#d9ef8b",
    "#ffffbf", "#fee999", "#fdc877", "#fca55a", "#f57547",
    "#e34a33", "#c22b26", "#a50026",
]
...
def _text_on(v: float) -> str:
    """배경색에 대해 흰 글씨와 먹 글씨 중 대비가 큰 쪽을 고른다.

    고정 임계값을 쓰면 램프를 바꿀 때마다 깨진다. 초록-노랑-빨강 램프는
    가운데(노랑)가 가장 밝아 임계 하나로는 맞출 수 없다.
    """
    L = _relative_luminance(risk_color(v))
    c_white = (1.05) / (L + 0.05)
    c_ink = (L + 0.05) / (0.05 + _relative_luminance((0.043, 0.043, 0.043)))
